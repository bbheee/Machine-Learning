...
def classifyBayes(X, prior, mu, sigma):
    ...
    for jdx in range(Nclasses):
        u = mu[jdx, :]
        delta = X - u
        sig = np.diag(sigma[jdx, :, :])
        inverse_sig= np.diag(1/sig)

        t1 = -0.5*np.log(np.linalg.det(sigma[jdx, :, :]))
        t2 = -0.5*np.diag(np.dot(np.dot(delta,inverse_sig),np.transpose(delta)))
        t3 = np.log(prior[jdx, :])
